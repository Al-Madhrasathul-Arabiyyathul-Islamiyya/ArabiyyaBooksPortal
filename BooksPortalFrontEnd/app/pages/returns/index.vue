    ...
    <div class="flex flex-wrap items-end justify-between gap-3">
      <div>
        ...
        <p class="text-sm text-surface-600 dark:text-surface-400">
          Track return slips and create new return records.
        </p>
      </div>
      <Button
        label="New Return"
        icon="pi pi-plus"
        @click="navigateTo('/returns/create')"
      />
    </div>
    ...
    <Card>
      <template #content>
        <div class="grid gap-3 md:grid-cols-4">
          ...
          />
          <FormsSearchInput
            id="returns-student-search"
            v-model="filters.studentSearch"
            name="returns-student-search"
            persist-key="bp.search.operations.returns.student"
...
import type { Lookup, ReturnSlip } from '~/types/entities'
...
async function loadSlips() {
  ...
  try {
    ...
    showError(response.message ?? 'Failed to load returns')
  }
  catch (error: unknown) {
    showError(getFriendlyErrorMessage(error, 'Failed to load returns'))
  }
  finally {
...
async function searchByReference() {
  ...
  try {
    const response = await api.get<ReturnSlip>(API.returns.byReference(referenceSearch.value))
    if (response.success) {
    ...
    showError(response.message ?? 'Return reference not found')
  }
  catch (error: unknown) {
    showError(getFriendlyErrorMessage(error, 'Return reference not found'))
  }
  finally {
...
function onRowClick(event: { data: ReturnSlip }) {
  void navigateTo(`/returns/${event.data.id}`)
}
