...
- [x] Phase 8 verification debt closure across targeted modules is implemented and verified.
...
### Partial
- [x] Slip lifecycle states (`Processing`, `Finalized`, `Cancelled`) are exposed with consistent guards across distribution/returns/teacher flows.
- [x] Revision workflows for processing slips are exposed where backend supports revision.
...
### Not started / pending
- [x] Final frontend support for backend follow-ups in `plans/04-high-priority-backend-followups.md` that affect operational screens.
...
- [x] Add explicit lifecycle state handling in relevant pages:
...
- [x] Add/complete processing-state revision UX where backend supports revision.

### Slice 3 - Frontend consistency pass
- [x] Ensure all table/list endpoints use consistent paginated adapters.
