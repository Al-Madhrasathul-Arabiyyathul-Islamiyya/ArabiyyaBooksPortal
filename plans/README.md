# Plans Index

## Canonical Active Plans (Use these)

| Order | Plan | Description |
|------:|------|-------------|
| 01 | [01-frontend-core-roadmap.md](./01-frontend-core-roadmap.md) | Primary frontend execution roadmap and verification debt closure |
| 02 | [02-frontend-validation-migration.md](./02-frontend-validation-migration.md) | Validation standardization roadmap (Regle + Zod) |
| 03 | [03-backend-pagination-enhancements.md](./03-backend-pagination-enhancements.md) | Backend pagination contract upgrades and frontend adoption tracking |
| 04 | [04-high-priority-backend-followups.md](./04-high-priority-backend-followups.md) | Slip lifecycle/revision and related backend follow-ups |
| 05 | [05-post-v1-backup-and-scheduled-reports.md](./05-post-v1-backup-and-scheduled-reports.md) | Post-v1 scheduler/backup/report automation plan |
| 06 | [06-backend-grade-bulk-import-history.md](./06-backend-grade-bulk-import-history.md) | Historical record of grade entity and bulk import rollout |
| 07 | [07-frontend-testing-suite-rollout.md](./07-frontend-testing-suite-rollout.md) | Modular rollout plan for frontend unit/e2e test coverage and gates |
| 08 | [08-post-v1-realtime-signalr-and-websockets.md](./08-post-v1-realtime-signalr-and-websockets.md) | Post-v1 realtime event streaming plan for bulk-import and operational monitoring |
| 99 | [99-plan-index-and-archive.md](./99-plan-index-and-archive.md) | Archive policy and plan-management conventions |

## Legacy Plan Files

Legacy filenames are retained as pointer stubs for continuity. Full original content is preserved in `plans/archive/`.
